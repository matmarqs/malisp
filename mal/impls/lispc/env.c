...
void mal_env_set(mal_env_t *env, string_t str_key, mal_obj_t mal_val) {
    ...
    mal_obj_t existing;
    if (env_table_get(env->data, str_key, &existing)) { // exists already, release it
        env_table_remove(env->data, str_key);
        mal_obj_release(&existing);
    }
    mal_obj_retain(&mal_val);
...
bool mal_env_get(mal_env_t *env, string_t str_key, mal_obj_t *mal_val) {
    while (env) {
        if (env_table_get(env->data, str_key, mal_val)) {
            mal_obj_retain(mal_val); // caller gets a retained reference
            return true;
        }
        env = env->outer;
    }
