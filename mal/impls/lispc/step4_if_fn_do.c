...
void mal_eval(mal_obj_t *node, mal_env_t *env) {
    ...
    switch (node->type) {
    case MAL_BUILTIN:
        switch (node->data.builtin.type) {
        case MAL_BUILTIN_BOOL:
            *node = mal_obj_num(node->data.builtin.data.builtin_bool == true);
            break;
        case MAL_BUILTIN_NIL:
            *node = mal_obj_num(0);
            break;
        case MAL_BUILTIN_NUMBER:
            *node = mal_obj_num(node->data.builtin.data.builtin_num);
            break;
        default:
            break;
        }
    ...
    case MAL_SYMBOL:
        ...
        *node = temp_obj;    // because we only do a shallow copy
        mal_eval(node, env); // additional eval, because we want to evaluate builtin's
        return;
        break;
    case MAL_LIST:
        list = node->data.list;
        if (!mal_list_is_empty(list)) {
            ...
            if (strncmp(first.data.symbol.str, "def!", 4) == 0) {
                ...
                mal_obj_t temp;
                MAL_ASSERT(node, (!mal_env_get(env, key, &temp) ||
                                  temp.type != MAL_BUILTIN),
                           "Error: Reserved symbol '%s' found in def! expression",
                           mal_obj_sprint(mal_list_get(list, 1)));
                mal_eval(value, env);
                ...
                for (int i = 0; i < mal_list_len(bindings); i += 2) {
                    ...
                    MAL_ENV_ASSERT(node, new_env, symbol->type == MAL_SYMBOL &&
                               (!mal_env_get(env, symbol->data.symbol, &temp) ||
                                temp.type != MAL_BUILTIN),
                               "Error: Reserved symbol '%s' found in let* expression",
            ...
            MAL_ASSERT(node,
                       func_symbol.type == MAL_BUILTIN &&
                       func_symbol.data.builtin.type == MAL_BUILTIN_FN,
                       "%s", mal_obj_sprint(&func_symbol));
            // pop the symbol, and let it act on the rest of the list
            mal_list_pop_front(list, NULL);
            func_symbol.data.builtin.data.builtin_fn(node);
            return;
        }
        break;
    default:
